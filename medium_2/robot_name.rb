=begin 
P:
Write a program that manages robot factory settings.

When robots come off the factory floor, they have no name.The first time you boot them up, a random name is generated, such as RX837 or BC811.

Every once in a while we need to reset a robot to its factory settings, which means that their name gets wiped. The next time you ask, it will respond with a new random name.

The names must be random: they should not follow a predictable sequence. Random names means a risk of collisions. Your solution should not allow the use of the same name twice when avoidable.
E:

Req:
  number has format XX000
  name is generated randomly without repeating
  name can be reset (regenerated) to provide a new random name that is different than others
D:
  Input =
  Output =

A:
   -
   -
   -
   -
   -
   -

C:

=end

class Robot
  ALPHABET = ('A'..'Z').to_a
  
  attr_reader :name

  def initialize
    @seeds = []
    reset
  end

  def reset
    @seeds.include?(srand) ? srand : (@seeds << srand)
    random_number = rand
    new_name = "#{ALPHABET[(rand * 26).floor] + ALPHABET[(rand**2 * 26).floor]}#{(rand * (999 - 100) + 100).floor}"
    new_name != name ? (@name = new_name) : generate
  end
end